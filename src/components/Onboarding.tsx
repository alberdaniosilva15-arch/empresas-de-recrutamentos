  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-premium-black/90 backdrop-blur-md">
      <motion.div 
        ...
        className="bg-premium-gray w-full max-w-md rounded-[2.5rem] p-10 text-center shadow-2xl border border-premium-border"
      >
        <div className={`w-24 h-24 rounded-3xl ${steps[step].color} flex items-center justify-center text-white mx-auto mb-10 shadow-xl shadow-current/20`}>
          <CurrentIcon size={48} />
        </div>
        
        <h3 className="text-3xl font-black text-gold mb-4 tracking-tight">{steps[step].title}</h3>
        <p className="text-slate-500 mb-12 leading-relaxed font-medium">{steps[step].description}</p>
